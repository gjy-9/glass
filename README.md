...
- **阿里云Qwen (Alibaba Cloud)**: 支持文本和视觉模型，API Key获取[阿里云DashScope](https://dashscope.console.aliyun.com/apiKey)
- Deepgram API: Get Deepgram API Key [here](https://console.deepgram.com/signup)
- Local LLM Ollama & Whisper

### 中文语音识别支持 (Chinese Speech Recognition)

Glass 现在支持中文语音识别！使用以下任一方法：

**方法 1: 使用启动脚本 (Windows)**
- 中文识别: 双击 `start-zh.cmd`
- 英文识别: 双击 `start-en.cmd`

**方法 2: 设置环境变量**
```bash
# 中文识别
set OPENAI_TRANSCRIBE_LANG=zh
npm start

# 英文识别 (默认)
set OPENAI_TRANSCRIBE_LANG=en
npm start
```
...
在项目根目录创建 `.env` 文件：
```
OPENAI_TRANSCRIBE_LANG=zh
```

**支持的 STT 提供商:**
- **Deepgram** (推荐) - 高质量中文识别
- **Gemini** - 支持中文
- **Whisper (本地)** - 支持中文 (需下载中文模型)
...
- **Feb 2**: Added Alibaba Cloud Qwen Vision Models support (qwen-vl-max, qwen-vl-plus, qwen-vl-turbo)
