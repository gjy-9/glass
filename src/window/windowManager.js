const { BrowserWindow, globalShortcut, screen, app, shell, ipcMain } = require('electron');
    ...
    const createFeatureWindow = (name) => {
        ...
        switch (name) {
            ...
            case 'ask': {
                ...
                // 设置截图上下文IPC接口（方案4实现）
                ask.webContents.on('dom-ready', () => {
                    ask.webContents.executeJavaScript(`
                        if (window.api && window.api.askView) {
                            console.log('[AskView] IPC接口已准备就绪');
                        }
                    `);
                });
...
function setupIpcHandlers(windowPool, layoutManager) {
    // 方案4：上下文更新IPC处理器
    ipcMain.handle('ask:contextUpdate', (event, data) => {
        const askWin = windowPool.get('ask');
        if (askWin && !askWin.isDestroyed()) {
            askWin.webContents.send('ask:contextUpdate', data);
        }
    });
