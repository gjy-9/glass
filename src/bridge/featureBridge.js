...
module.exports = {
  ...
  initialize() {
    ...
    ipcMain.handle('ask:clearScreenshotContext', async () => {
        askService.clearScreenshotContext();
        return { success: true };
    });
    ipcMain.handle('ask:getScreenshotContext', async () => {
        // 从askService获取上下文数据
        const contextData = askService.getScreenshotContext ? askService.getScreenshotContext() : 
            { accumulatedScreenshots: [], accumulatedText: '', lastScrollPosition: 0 };
        return {
            screenshotCount: contextData.accumulatedScreenshots ? contextData.accumulatedScreenshots.length : 0,
            hasContext: contextData.accumulatedScreenshots && contextData.accumulatedScreenshots.length > 0,
        };
    });
