...
export class AskView extends LitElement {
    ...
    connectedCallback() {
        ...
        // 方案4：处理上下文更新
        this.handleContextUpdate = (event, contextData) => {
            this.screenshotCount = contextData.screenshotCount;
            this.hasContext = contextData.hasContext;
            this.requestUpdate(); // 重新渲染UI
        };
        ...
        if (window.api) {
            ...
            // 方案4：监听上下文更新
            window.api.askView.onContextUpdate(this.handleContextUpdate);
    ...
    async handleClearContext() {
        try {
            if (window.api) {
                await window.api.askView.clearScreenshotContext();
                this.screenshotCount = 0;
                this.hasContext = false;
                console.log('截图上下文已清除');
            }
        } catch (error) {
    ...
    updated(changedProperties) {
        ...
        if (changedProperties.has('isLoading') || changedProperties.has('currentResponse')) {
            console.log('[AskView] Updated triggered, changed properties:', Array.from(changedProperties.keys()));
            console.log('[AskView] currentResponse length:', this.currentResponse?.length || 0);
            console.log('[AskView] isLoading:', this.isLoading, 'isStreaming:', this.isStreaming);
            this.renderContent();
        }
                ...
                <div class="response-header ${!hasResponse ? 'hidden' : ''}">
                    <div class="header-left">
                        ...
                        ${this.screenshotCount > 0 ? html`
                            <div class="context-indicator" title="已捕获 ${this.screenshotCount} 个屏幕">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    <rect x="3" y="3" width="18" height="18" rx="2" ry="2" />
                                    <line x1="3" y1="9" x2="21" y2="9" />
                                    <line x1="9" y1="21" x2="9" y2="9" />
                                </svg>
                                <span class="context-count">${this.screenshotCount}</span>
                            </div>
                        ` : ''}
                    </div>
                    <div class="header-right">
                        ...
                        <div class="header-controls">
                            <!-- 方案4：清除上下文按钮 -->
                            ${this.screenshotCount > 0 ? html`
                                <button class="clear-context-btn" @click=${this.handleClearContext} title="清除上下文">
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
