...
class ModelStateService extends EventEmitter {
    ...
    async getAvailableModels(type) {
        ...
        // 首先添加所有定义在 PROVIDERS 中的模型（包括未配置的提供商）
        for (const providerId in PROVIDERS) {
            if (PROVIDERS[providerId]?.[modelListKey]) {
                // 对于 ollama，从数据库获取已安装模型
                if (providerId === 'ollama' && type === 'llm') {
                    try {
                        ...
                        if (installed && installed.length > 0) {
                            ...
                            // 如果没有已安装模型，显示 factory 中定义的所有 Ollama 模型
                            available.push(...PROVIDERS[providerId][modelListKey]);
                        }
                    } catch (err) {
                        // 如果数据库出错，回退到 factory 中定义的模型
                        console.warn('[ModelStateService] Failed to get Ollama models from DB, using factory defaults');
                        available.push(...PROVIDERS[providerId][modelListKey]);
                    }
                } else {
                    available.push(...PROVIDERS[providerId][modelListKey]);
